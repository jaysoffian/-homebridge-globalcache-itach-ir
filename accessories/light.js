...
class LightAccessory extends SwitchAccessory {
    ...
    if (exclusives) {
      exclusives.forEach((exname) => {
        ...
        if (exAccessory && exAccessory.config.type === "light") {
          ...
          if (!exAccessory.exclusives) {
            exAccessory.exclusives = [];
          }
          ...
          log(
            `${name}: No light accessory could be found with the name "${exname}". Please update the "exclusives" value or add matching light accessories.`
          );
        }
      });
    }
  ...
  async setSwitchState(irData, previousValue) {
    ...
    if (state.switchState) {
      if (this.exclusives) {
        this.exclusives.forEach((x) => {
          if (x.state.switchState) {
            log(
              `${name} setSwitchState: (${x.name} is configured to be turned off)`
            );
            x.reset();
      ...
      const brightness =
        useLastKnownBrightness && state.brightness > 0
          ? state.brightness
          : defaultBrightness;
      if (
        brightness !== state.brightness ||
        previousValue !== state.switchState
      ) {
  ...
  async setHue() {
    await catchDelayCancelError(async () => {
      ...
      const closest = foundValues.reduce((prev, curr) =>
        Math.abs(curr - state.hue) < Math.abs(prev - state.hue) ? curr : prev
      );
      var irData = "";
    ...
    await catchDelayCancelError(async () => {
      ...
      if (state.brightness > 0) {
        ...
        assert(
          ...
          `\x1b[31m[CONFIG ERROR] \x1b[33mbrightness\x1b[0m keys need to ne set. See the config-sample.json file for an example.`
        );

        const closest = foundValues.reduce((prev, curr) =>
          Math.abs(curr - state.brightness) < Math.abs(prev - state.brightness)
            ? curr
            : prev
        );
        const irData = data[`brightness${closest}`];
